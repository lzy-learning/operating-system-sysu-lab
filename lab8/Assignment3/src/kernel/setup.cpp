...
void first_process()
{
    ...
    else
    {
        if (pid)
        {
            printf("father process, first fork reutrn: %d\n", pid);
            pid = fork();
            if (pid)
            {
                printf("father process, second fork return: %d\n", pid);
            }
            else
            {
                printf("second child process, fork return: %d, my pid: %d\n", pid, programManager.running->pid);
            }
        }

        else
            printf("first child process, fork return: %d, my pid: %d\n", pid, programManager.running->pid);
    }
