...
void ProgramManager::schedule(bool releaseCorpseProcess)
{
    bool status = interruptManager.getInterruptStatus();
    interruptManager.disableInterrupt();
    ...
    {
        // 回收线程，子进程留到父进程回收
        if(!running->pageDirectoryAddress || releaseCorpseProcess) {
            printf("thread or zombie release its PCB\n");
            releasePCB(running);
        }
    }
...
void ProgramManager::exit(int ret)
{
    ...
    // 如果有子进程，那么将子进程PCB内的parentID设置为-1
    for(int i = 0;i<MAX_PROGRAM_AMOUNT;i++){
        if(PCB_SET_STATUS[i]){
            if(((PCB*)((int)PCB_SET+i*PCB_SIZE))->parentPid == program->pid){
                ((PCB*)((int)PCB_SET+i*PCB_SIZE))->parentPid = -1;
            }
        }
    }

    // 如果自己是子进程，而且后于父进程退出，那么它负责清理空间
    if(program->parentPid == -1){
        printf("pid: %d: my father process has exited\n", program->pid);
        schedule(true);
    }
    else schedule();
